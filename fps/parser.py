...
class FPSParser(object):
    def __init__(self, fps_path=None, string_data=None):
        if fps_path:
            ...
            self._etree = ET.fromstring(string_data).getroot()
        else:
            raise ValueError(
    ...
    def _parse_one_problem(self, node):
        ...
        for item in node:
            ...
            if tag in (
                ...
                if not lang:
                    raise ValueError(
                        "Invalid " + tag + ", language name is missed")
...
if __name__ == "__main__":
    ...
    parser = FPSParser("test.xml")
    helper = FPSHelper()
    problems = parser.parse()
    for index, problem in enumerate(problems):
        path = os.path.join("/tmp/", str(index + 2))
        os.mkdir(path)
