...
mkdir -p $DATA/log $DATA/config $DATA/ssl $DATA/test_case $DATA/public/upload $DATA/public/avatar $DATA/public/website $DATA/model_file $DATA/contest_rank_file

# -f filename 判断是否为常规文件

if [ ! -f "$DATA/config/secret.key" ]
then
echo $(cat /dev/urandom | head -1 | md5sum | head -c 32) > "$DATA/config/secret.key"
fi

if [ ! -f "$DATA/public/avatar/default.png" ]
then
cp data/public/avatar/default.png $DATA/public/avatar
fi

if [ ! -f "$DATA/public/website/favicon.ico" ]
then
cp data/public/website/favicon.ico $DATA/public/website
fi

if [ ! -f "$DATA/config/rsa_pri_key.pem" ]
then
cp data/config/rsa_pri_key.pem $DATA/config
fi

if [ ! -f "$DATA/config/rsa_pub_key.pem" ]
then
cp data/config/rsa_pub_key.pem $DATA/config
fi

if [ ! -f "$DATA/model_file/import_users.xlsx" ]
then
cp data/model_file/import_users.xlsx $DATA/model_file
fi
...
if [ ! -z "$LOWER_IP_HEADER" ]
then
sed -i r"s/__IP_HEADER__/\$http_$LOWER_IP_HEADER/g" api_proxy.conf
else
sed -i r"s/__IP_HEADER__/\$remote_addr/g" api_proxy.conf
fi
